...
def get_sobel_edges(mask):
    return kornia.filters.sobel(mask)
...
class ViewDisentangleNetwork(nn.Module):
    ...
    def forward(self, image, view=None, view_rand=None, view_txt=None):
        N = image.shape[0]
        if view is None and view_rand is None:
            ...
            zv_text_feat = F.normalize(view_txt,dim=-1)
...
class ViewDisentangleModel(BaseModel):
    ...
    def forward(self):
        ...
        view_txt_ab = torch.cat([self.data_view_feat_a, self.data_view_feat_a], dim=0)
        ...
        out = self.netFull(image_ab, view=view_ab, view_rand=view_ab_rand,view_txt=view_txt_ab)
    ...
    def forward_test(self):
        ...
        out = self.netFull(self.data_image, view=self.data_view,view_txt=self.data_view_feat)
    ...
    def forward_inference(self):
        ...
        if self.opt.view is None:
            ...
            self.data_view = self.encode_view(self.data_view)
        # out = self.netFull(self.data_image, view=self.data_view)
        out = self.netFull(self.data_image)
        for k, v in out.items():
            setattr(self, f"out_{k}", v)
    ...
    def backward(self):
        ...
        # logits = torch.matmul(F.normalize(self.out_zv_pred, dim=-1), F.normalize(self.out_zv_text_feat, dim=-1).T)
        # labels = torch.arange(logits.size(0), device=logits.device)
        # self.loss_view_clip = F.cross_entropy(logits, labels)
        self.loss_view_clip = 0
        ...
        self.loss_tot = self.loss_iou_tot + self.opt.lambda_iou_rand * self.loss_iou_rand_tot + \
            ...
            self.opt.lambda_sd * self.loss_sd + 0.05 * self.loss_view_clip
