        ...
        if epoch % opt.val_epoch_freq == 0:
            ...
            with torch.no_grad():
                for class_id in opt.class_id:
                    ...
                    val_losses = model.get_current_losses('val')
                    for loss_name, loss_val in val_losses.items():
                        writer.add_scalar(f"val_{class_id}_{loss_name}", loss_val, global_step=epoch)
                    print(f"[{class_id}] Validation losses |", ' '.join([f"{k}: {v:.3e}" for k, v in val_losses.items()]))
        ...
        if epoch % opt.test_epoch_freq == 0:
            ...
            with torch.no_grad():
                for class_id in opt.class_id:
                    ...
                    for loss_name, loss_val in val_losses.items():
                        writer.add_scalar(f"test_{class_id}_{loss_name}", loss_val, global_step=epoch)
                    print(f"[{class_id}] Test losses |", ' '.join([f"{k}: {v:.3e}" for k, v in val_losses.items()]))
